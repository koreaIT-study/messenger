...
#SERVER_HOME='/home/ec2-user/deploy/messenger-client'
if [ "${SERVER_HOME:-}" = "" ];
...
JAR_FILE=`find $SERVER_HOME -name messenger_client-*.jar`
echo "server JAR file : $JAR_FILE"

print_usage() {
  echo "messenger_client usage:"
  echo "    ./messenger_client.sh [option arguments] [start|stop|restart]"
  echo "    --help, -h, ?)"
...
stop_server() {
  PROC=`ps aux | grep messenger_client- | grep spring`
  if [[ $PROC == *"messenger_client-"* ]]; then
    echo "Process is running."
    PID=`ps -ef | grep messenger_client- | grep spring | grep -v grep | awk '{print $2}'`
    kill -9 $PID
    wait_graceful_shutdown $PID
  else
...
JVM_OPTS="-Xms4096m -Xmx4096m -server -XX:+UseG1GC -XX:MaxGCPauseMillis=200 -XX:+DisableExplicitGC \
          -verbose:gc -XX:+PrintGCDetails -XX:-PrintGCTimeStamps -Xloggc:$SERVER_LOGS/messenger_client-gc.log \
